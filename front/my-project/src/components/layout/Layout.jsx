...
const Layout = ({ children }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-100">
      <nav className="w-full md:w-64 bg-gray-900 text-gray-300">
        <div className="p-4 border-b border-gray-800 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <Menu size={22} />
            <h1 className="text-lg font-semibold text-white">販売管理システム</h1>
          </div>
          <button className="md:hidden" onClick={() => toggleMenu('main')}>
            <Menu size={22} />
          </button>
        </div>

        <ul className={`p-2 ${openMenus['main'] ? 'block' : 'hidden'} md:block`}>
          {menuItems.map((item) => (
